...
public class GameListAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		try {
			...
			// list.get(position).get(Config.KEY_GAME_ID))));
			// }
			tvPersonNum.setText(list.get(position).getString(Config.KEY_GAME_MAX_ATTEND));
			String[] str = list.get(position).getString(Config.KEY_GAME_START_TIME).split("-");
			tvGameTimeGameList.setText(str[0]+"."+str[1] + "." + str[2]);
			
			//TODO
			String[] str2 = list.get(position).getString(Config.KEY_GAME_END_TIME).split("-");
			tvGameTimeEndGameList.setText(str2[0]+"."+str2[1] + "." + str2[2]);
			
			if (list.get(position).getString(Config.KEY_GAME_STATUS).equals("接受报名")) {
				tvGameStatusGameList.setImageResource(R.drawable.weibaoming);
			}
			if (list.get(position).getString(Config.KEY_GAME_STATUS).equals("未开始")) {
				tvGameStatusGameList.setImageResource(R.drawable.weikaishi);
			}
			if (list.get(position).getString(Config.KEY_GAME_STATUS).equals("报名结束")) {
